 ...
 * 
 */
import { denosass, ensureDirSync, emptyDirSync, walkSync, path} from "./deps.ts"
import { SassObject, SassFormats, ExportOptions, InputType, SassOptions} from "./types/module.types.ts"
export const warn = (msg: string) => console.warn(`🟡 %c[%cSass%c]%c ${msg}`, 'color: yellow', 'color: orange', 'color: yellow', 'color: yellow;')
export const error = (msg: string) => console.error(`🛑 %c[%cSass%c]%c ${msg}`, 'color: yellow', 'color: red', 'color: yellow', 'color: red;')
export const log = (msg: string) => console.log(`🔵%c[%cSass%c]%c ${msg}`, 'color: red', 'color: cyan', 'color: red', 'color: gray;')
...
class Sass implements SassObject {
		...
		if (typeof this.#current === "string" || this.#current instanceof Map) {
			if (this.#outmode === 1) {
				if (this.#mode === "string") {
					...
					this.output = [...(this.#current as Map<string, string>)].reduce((acc, file) => {
	...
	public to_buffer (format: SassFormats = "compressed") {
		...
		if (this.#outmode === 1) {
			if (this.#mode === "string") {
				this.output = this.encoder.encode(denosass.str(this.#current as string, format))
			} else {
				this.output = this.encoder.encode(denosass.file(this.#current as string, format))
			}
		}  else if (this.#outmode === 2) {
				this.output = [...(this.#current as Map<string, string>)].reduce((acc, file) => {
			...
			if (outputOptions.destFile) {
				...
				if (this.output instanceof Map) {
					this.output.forEach((ParsedCSs) => {
						Deno.writeTextFileSync(fileURL, typeof ParsedCSs !== "string" ? this.decoder.decode(ParsedCSs): ParsedCSs, { append: true, create: true, mode: 644})
					})
				} else {
					Deno.writeTextFileSync(fileURL, typeof this.output !== "string" ? this.decoder.decode(this.output): this.output, { append: false, create: true, mode: 644})
				}
			} else {
				...
				if (this.output instanceof Map) {
					this.output.forEach((ParsedCSs, filename) => {
						...
						Deno.writeTextFileSync(fileURL, typeof ParsedCSs !== "string" ? this.decoder.decode(ParsedCSs): ParsedCSs, { append: false, create: true, mode: 644})
					})
				} else {
					const filepath = path.format({
						root: './',
	...
	#processDir () {
		...
		for (const entry of walkSync(this.#input as string, {
			maxDepth: 1,
			includeDirs: false,
			exts: [".scss", ".sass"]
		})) {
	...
	#processObject () {
		...
		urls.map((filePath) => {
				if (exists(filePath, "dir")) {
					...
					for (const entry of walkSync(filePath, {maxDepth: 1, exts: [".scss", ".sass"]})) {
						this.#current.set(path.parse(entry.path).name, entry.path)
					}
